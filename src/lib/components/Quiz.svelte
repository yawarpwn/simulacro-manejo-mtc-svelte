<script>
	...
	import { globalState, endApp } from '$lib/store/questions.svelte';
...
<div class="mx-auto h-full w-full max-w-3xl xl:max-w-7xl">
	...
	{#if !showResult}
		<form onsubmit={handleSubmit} class="fixed bottom-0 left-0 right-0 h-[76px]">
			...
			>
				<button
					class="bg-primary h-full w-full cursor-pointer rounded-md px-4 py-2 text-black disabled:pointer-events-none disabled:opacity-20"
	...
	{:else}
		<form onsubmit={handleNextQuestion} class="fixed bottom-0 left-0 right-0 h-[76px]">
			...
			>
				<button
					class="bg-primary flex h-full cursor-pointer items-center gap-2 rounded-md px-4 py-2 text-black disabled:pointer-events-none"
					>Siguiente <svg
						width="1em"
						height="1em"
						fill="none"
